...
void Chip8::LoadROM(char const* file_name)
{
	...
	if (file.is_open()) {
		std::cout << "WORKING AS INTENDED" << std::endl;
		...
		std::cout << "NOT WORKING AS INTENDED" << std::endl;
	}
}
...
void Chip8::OP_Dxyn()
{
	...
	for (unsigned int row = 0; row < height; row++) {
		...
		for (unsigned int col = 0; col < 8; col++) // byte means 8px wide
		{
...
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
	for (uint8_t val = 0; val <= Vx; val++) {
		memory[index + val] = registers[val];
	}
}
...
void Chip8::OP_Fx65()
{
	uint8_t Vx = (opcode & 0x0F00u) >> 8u;
	for (uint8_t val = 0; val <= Vx; val++) {
		registers[val] = memory[index + val];
	}
}
